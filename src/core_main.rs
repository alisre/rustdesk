...
pub fn core_main() -> Option<Vec<String>> {
    // Check for early-exit commands BEFORE global_init to avoid GUI initialization
    ...
    if args_vec.len() > 1 {
        match args_vec[1].as_str() {
            ...
            "--login" => {
                ...
                let username = args_vec.iter().position(|x| x == "--username" || x == "-u")
                    .and_then(|i| args_vec.get(i + 1))
                    .map(|s| s.as_str())
                    .unwrap_or("");
                let password = args_vec.iter().position(|x| x == "--login-password")
                    .and_then(|i| args_vec.get(i + 1))
                    .map(|s| s.as_str())
                    .unwrap_or("");
